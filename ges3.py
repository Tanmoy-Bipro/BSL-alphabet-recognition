...
def img_cap():
    count1 = 0
    path1 = 'F:/Image Collect from videoFeed/Dataset/K/'
    count2 = 0
    path2 = 'F:/Image Collect from videoFeed/Dataset/K/'
    count3 = 0
    path3 = 'F:/Image Collect from videoFeed/Dataset/K/Kbin/'
    while (True):
        fname1 = "a" + str(count1) + ".jpg"
        cv2.imwrite(os.path.join(path1, fname1), skinflip)
        count1 += 1
        fname2 = "b" + str(count2) + ".jpg"
        cv2.imwrite(os.path.join(path2, fname2), originalflip)
        count2 += 1
        fname3 = "a" + str(count3) + ".jpg"
        cv2.imwrite(os.path.join(path3, fname3), binaryflip)
        count3 += 1
        print(count)
        time.sleep(0.05)
...
if __name__ == "__main__":
    ...
    top, right, bottom, left = 62, 300, 300, 590
    length = left-right
    width = bottom-top
    ...
    while(True):
        ...
        if num_frames < 30:
            ...
            if hand is not None:
                ...
                originalflip = cv2.flip(rgb, 1)
                binaryflip = cv2.flip(thresholded, 1)
